...
def parse_args():
    ...
    parser.add_argument(
        "--control-mode",
        type=str,
        choices=["nmpc", "pid", "policy"],
        default="nmpc",
        help="Controller used after PPO inference: "
             "nmpc (default), pid (speed PID + policy steer), policy (policy outputs only)"
    )
    parser.add_argument(
        "--results-path",
        type=str,
...
if __name__ == '__main__':
    ...
    # per-route metrics for CSV export
    results = []
    ...
    for i in range(0, num_routes):
        ...
        if args.generate_saliency:
            ...
            os.makedirs(os.path.join(saliency_dir, 'right'), exist_ok=True)
            os.makedirs(os.path.join(saliency_dir, 'front'), exist_ok=True)
            os.makedirs(os.path.join(saliency_dir, 'left'), exist_ok=True)
        ...
        while not done:
            ...
            command_tensor = torch.tensor(command, dtype=torch.float).to(device)
            speed_tensor = torch.tensor([speed], dtype=torch.float).to(device)

            action, _, _, _ = agent.get_action_and_value(
                obs.unsqueeze(0),
                command_tensor.unsqueeze(0),
                speed_tensor.unsqueeze(0),
            ...
            # -------- controller selection / NMPC + PID fallback --------
            nmpc_plan = None
            ...
            if args.control_mode == "nmpc":
                ...
                if nmpc_plan.success:
                    ...
                    # no route left → just use policy action
                    applied_action = base_action
                else:
                    # NMPC 실패 시 속도 PID + 정책 조향 fallback
            ...
            if done:
                ...
                print(
                    f'route {i:02}: {info} | mode {plan_status} | '
                    f'tracking {mean_error:.2f}m | control Δ {mean_smoothing:.3f} | '
                    f'fallbacks {len(safety_events)}'
                )
